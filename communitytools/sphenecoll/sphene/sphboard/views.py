...
def options(request, thread_id, group = None):
    ...
    if request.GET['cmd'] == 'makeSticky':
        thread.set_sticky(True)
    elif request.GET['cmd'] == 'removeSticky':
        thread.set_sticky(False)
    elif request.GET['cmd'] == 'toggleClosed':
        thread.set_closed(not thread.is_closed())
    elif request.GET['cmd'] == 'modifytags':
...
class MoveAndAnnotateForm(MoveForm, AnnotateForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['body'].help_text = string_concat(ugettext_lazy(u'Please describe why this query had to be moved.'), ' ', self.fields['body'].help_text)
