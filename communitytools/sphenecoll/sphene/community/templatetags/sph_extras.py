...
class NewsRSSLinkMacro (object):
    ...
    def handleMacroCall(self, doc, params):
        ...
        return HTML( '<a href="%(url)s"><img src="%(media_url)ssphene/community/icons/feed-icon-14x14.png" border="0" alt="RSS Feed of latest queries" title="RSS Feed of latest queries" /></a>' % { 'url': url, 'media_url': settings.MEDIA_URL } )
...
@register.inclusion_tag('_display_username.html')
...
@register.filter
def mainsite_user_profile_link(value):
    """ Returns the URL to the user profile. """
#    req = get_current_request()
#    urlconf = getattr(req, 'urlconf', None)
    return '/accounts/profile/%d/'%(value.id,)
#    return reverse('sphene.community.views.profile', urlconf, (), { 'user_id': value.id } )
...
def sph_showavatar(user):
    ...
    return '<img src="%s" width="%dpx" height="%dpx" alt="%s" class="sph_avatar"></img>' % (avatar, avatar_width, avatar_height, _(u'Users photo'))
