...
def mainsite_profile_edit_init_form(sender, instance, signal, request, *args, **kwargs):
    user = instance.user
    try:
        profile = CommunityUserProfile.objects.get( user = user, )
    except CommunityUserProfile.DoesNotExist:
...
def mainsite_profile_edit_save_form(sender, instance, signal, request, *args, **kwargs):
    data = instance.cleaned_data
    user = instance.user
    try:
        profile = CommunityUserProfile.objects.get( user = user, )
    ...
    request.user.message_set.create( message = _("Successfully saved user profile.") )
...
mainsite_edit_init_form.connect(mainsite_profile_edit_init_form, sender = EditProfileForm)
profile_edit_save_form.connect(community_profile_edit_save_form, sender = EditProfileForm)
mainsite_edit_save_form.connect(mainsite_profile_edit_save_form, sender = EditProfileForm)
